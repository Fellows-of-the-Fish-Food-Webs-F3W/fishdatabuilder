...
# filter batch, measurement, operation based on this

# Remove NA site
op_filtered <- op %>%
  filter(!(is.na(site_id) | is.na(operation_id))) %>%
...
op_filtered %>%
  filter(operation_id %in% id)


fish_batch %>%
  filter(prelevement_id %in% ele_sampling_selection) %>%
  filter(operation_id %in% op_filtered$operation_id)
ind_measure %>%
  filter(prelevement_id %in% ele_sampling_selection) %>%
  filter(operation_id %in% op_filtered$operation_id)
