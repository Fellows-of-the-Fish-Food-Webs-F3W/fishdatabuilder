...
fish_batch <- get_fish_batch_aspe() |> as_tibble()
individual_measurement <- get_individual_measurement_aspe() |> as_tibble()
```

```r

ti <- clean_fish_batch(fish_batch = fish_batch)

# HERE
ind_measure <- tbl(con, in_schema("aspe", "mesure_individuelle"))
arrange(ind_measure, mei_lop_id)

cleaning_batch_type_aspe() |> as_tibble()
fish_batch %>%
  left_join(select(species, species_id, species_code),
    by = join_by(lop_esp_id == species_id)) %>%
  left_join(select())
