...
test_that("check_aspe_fish_species handles FishBase validation", {
  ...
  expect_equal(
    result$missing_in_fishbase,
    "BAD"
  )

  expect_equal(
    sort(result$valid_codes),
    sort(c("ABL", "TRT"))
  )
})
...
test_that("check_aspe_fish_species handles empty inputs", {
  test_ref <- data.frame(
...
test_that("sanitize_species_code removes unwanted species", {
  ...
  expect_false(
    "ASL" %in% result$species_code
  )

  expect_false(
    "PFL" %in% result$species_code
  )
})
...
test_that("species_code_to_remove returns correct codes", {
  ...
  # Test not_fish = FALSE
  no_fish_false <- species_code_to_remove(
    not_fish = FALSE
  )
  ...
  # Test both FALSE
  both_false <- species_code_to_remove(
    FALSE,
    FALSE
  )
  ...
  add_code <- species_code_to_remove(
    FALSE,
    FALSE,
    additional_codes = c("ABA")
  )
...
test_that("species_code_to_replace returns correct mapping", {
  ...
  # Check that all replacement codes exist in the species reference
  expect_warning(
    species_ref <- cleaning_species_ref_aspe(),
    "Missing in FishBase"
  )

  valid_codes <- species_ref |>
    dplyr::pull(species_code)

  replacement_codes <- unname(mapping)

  expect_true(
    all(replacement_codes %in% valid_codes)
  )
})
